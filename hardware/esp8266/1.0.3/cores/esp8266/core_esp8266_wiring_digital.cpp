...
// #include "pins_arduino.h"  //y21m01d26 kxn remove to Arduino.h
#include "c_types.h"
#include "eagle_soc.h"
#include "ets_sys.h"
...
extern "C" {
  #ifdef VIET_ESP8266_V1
    uint8_t digital_pin_array[] = {
    IO_2,
    IO_15,// kxn dit y20m12d28
    IO_0, // kxn dit y20m12d28
    IO_16,
    IO_13,
    IO_12,
    IO_14
...
extern void __pinMode(uint8_t pin_in, uint8_t mode) {
  uint8_t pin;
  #ifdef VIET_ESP8266_V1
    if(pin_in >= 100){
      pin = digital_pin_array[pin_in - VIET_OFFSET_PIN];
    }else{
      pin = pin_in;
    }
  #endif
...
extern void ICACHE_RAM_ATTR __digitalWrite(uint8_t pin_in, uint8_t val) {
  uint8_t pin;
  #ifdef VIET_ESP8266_V1
    if(pin_in >= 100){
      pin = digital_pin_array[pin_in - VIET_OFFSET_PIN];
...
extern int ICACHE_RAM_ATTR __digitalRead(uint8_t pin_in) {
  uint8_t pin;
  #ifdef VIET_ESP8266_V1
    if(pin_in >= 100){
      pin = digital_pin_array[pin_in - VIET_OFFSET_PIN];
